...
hgsnat <-
  read_excel("EthoVision 14 dpf hgsnat.xlsx") %>%
  ...
  meta.hgsnat <-
  read_excel("genotype hgsnat 14dpf.xlsx") %>%
...
totaldist.hgsnat %>% saveRDS("C.14.totaldist.rds")
...
hgsnat %>% saveRDS("C.14.fulldata.rds")
...
CplotTrial(dataset = hgsnat, Dependent = hgsnat$Distance_travelled)
...
model_distance <- totaldist.hgsnat %>%
  group_by(fish_id) %>%
  ungroup() %>%
  unique() %>%
  droplevels() %>%
...
check_overdispersion(glm_crosses1) #hella overdispersed 
