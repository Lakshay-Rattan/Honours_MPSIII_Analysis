...
meta.sgsh.14 <-
  read_excel("14dpf sgsh genotype.xlsx") %>%
...
t.sgsh.14 <-
  meta.sgsh.14 %>%
  left_join(t.sgsh.14, by = c("Trial", "Position")) %>%
  na.omit
...
geno.props.plot(input = totaldist.sgsh.14, xaxis = "Genotype")
...
totaldist.sgsh.14 %>% saveRDS("A.14.totaldist.rds")

t.sgsh.14 %>% saveRDS("A.14.fulldata.rds")
...
Cplotdistance <- function(dataset, Dependent){
  ...
  ggplot(data = dataset, aes(x = Genotype, y = Dependent)) +
    ...
    scale_colour_viridis_d(option = "inferno", end = 0.75) +
...
Cplotdistance(dataset = totaldist.sgsh.14, Dependent = totaldist.sgsh.14$Total_distance) 



Cplotdistance(dataset = t.sgsh.14, Dependent = t.sgsh.14$Distance_travelled) +
  facet_wrap(~Bin)
...
CplotTrial(dataset = totaldist.sgsh.14, Dependent = totaldist.sgsh.14$Total_distance)


```


Overall distance travelled statistical analysis Genotype 
```{r Statistical analysis overall distance}

model_distance <- totaldist.sgsh.14 %>%
  group_by(fish_id) %>%
  ungroup() %>%
  unique() %>%
  droplevels() %>%
  mutate(gTrial = interaction(Genotype, Trial, drop = TRUE)) %>%
  lmer(formula = Total_distance ~ Genotype + (1|gTrial)) 
...
print(emmeans(model_distance, ~ Genotype), type ="emmean") %>%
  ...
  scale_color_viridis_d(end = 0.75, option = "inferno") +
...
#hard so say if the data is normal. Need to chat to Karissa.
lm_bin <- t.sgsh.14 %>%  
  mutate(gTrial = interaction(Genotype, Trial, drop = TRUE), 
         fish_id = as.character(fish_id)) %>% # 
  glmer(formula = Distance_travelled ~ (Bin*Genotype) + (1|gTrial) + (1|fish_id), #as per steve
       data = .,
...
glm_crosses <-
  ...
  mutate(gTrial = interaction(Genotype, Trial, drop = TRUE), 
         fish_id = as.character(fish_id)) %>% 
  glmer.nb(Freq_in_centre ~ (Bin*Genotype) + (1|gTrial) + (1|fish_id),
           data = .,
           family = Poisson (link = "log"))
...
print(emmeans(glm_crosses, ~ Genotype), type ="response") %>%
  ...
  labs(y = "model-predicted crosses into centre",
       x = "Genotype") +
  theme(text = element_text(size = 20))
...
```
SO homs consistently had lower n of crosses into centre compared to other genos. Lets look at how long the spent in the centre. same seen in hgsnat 5dpf.
