...
t.sgsh.14 %>%
  rename(
    ...
    mutate(Bin = factor(Bin, levels = c("Start-0:10:00", "0:10:00-0:20:00", "0:20:00-0:30:00", "0:30:00-0:40:00", "0:40:00-0:50:00","0:50:00-1:00:00")),
...
#These plates were empty or had bad tracking
#something sussy is going on with fish_id 7, so ill omit for now
...
Gotta double check the qqplot and ask about how im getting wald chi (Karissa)
...
#KARISSAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
Anova(lm_bin) %>%
  as.data.frame() %>%
  dplyr::rename(pval = `Pr(>Chisq)`) %>%
  kable(caption = "Type II Wald chisquare test of LME") %>%
